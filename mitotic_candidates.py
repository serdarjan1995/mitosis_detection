...
for directory in glob.glob(PATH_MAIN+'*/'):
    ...
    for csv_file_in_dir in glob.glob(directory+'*.csv'):
        ...
        #resize images
        resize_val = int(norm_disc_image.shape[0]/4)
        ...
        img_concat1 = np.concatenate((images[0], images[1],
                                     images[2], images[3],
                                     images[4], images[5]), axis=1)
        img_concat2 = np.concatenate((images[6], images[7],
                                      images[8], images[9],
                                      norm_disc_image, image_mask), axis=1)
        img_concat_all = np.concatenate((img_concat1,img_concat2), axis=0)
        
        while True:
            cv2.imshow(image_name,img_concat_all)
            key_to_quit = cv2.waitKey(0)
            cv2.destroyAllWindows()
            if key_to_quit==27:    # Esc key to stop
                break
            elif key_to_quit == ord('s'): # wait for 's' key to save and exit
                cv2.imwrite('image_concat.bmp',img_concat_all)
                cv2.destroyAllWindows()
